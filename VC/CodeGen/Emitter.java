...
public final class Emitter implements Visitor {
  ...
  public Object visitIfStmt(IfStmt ast, Object o) {
	Frame frame = (Frame) o;
	String fail = frame.getNewLabel();
	String pass = frame.getNewLabel();
  ...
  public Object visitWhileStmt(WhileStmt ast, Object o) {
	Frame frame = (Frame) o;
	String s = frame.getNewLabel();
	String f = frame.getNewLabel();

	frame.conStack.push(s);
	frame.brkStack.push(f);
  ...
  public Object visitForStmt(ForStmt ast, Object o) {
	...
	String s = frame.getNewLabel();
	String f = frame.getNewLabel();
	String i = frame.getNewLabel();

	frame.conStack.push(i);
	frame.brkStack.push(f);
  ...
  */
	 ...
	 ast.E.visit(this,o);
	 if (ast.E.type.isIntType() || ast.E.type.isBooleanType()) emit(JVM.IRETURN);
	 else if (ast.E.type.isFloatType()) emit(JVM.FRETURN);
     return null;
  }
  ...
  public Object visitBinaryExpr(BinaryExpr ast, Object o) {
	...
	if (op.charAt(1) == '+' || op.charAt(1) == '-' || op.charAt(1) == '*' || op.charAt(1) == '/') {
		frame.pop(2);
		if (op.equals("i+")) emit(JVM.IADD);
		else if (op.equals("i-")) emit(JVM.ISUB);
		else if (op.equals("i*")) emit(JVM.IMUL);
		else if (op.equals("i/")) emit(JVM.IDIV);
		else if (op.equals("f+")) emit(JVM.FADD);
		else if (op.equals("f-")) emit(JVM.FSUB);
		else if (op.equals("f*")) emit(JVM.FMUL);
		else if (op.equals("f/")) emit(JVM.FDIV);
		frame.push();
  ...
  public Object visitAssignExpr(AssignExpr ast, Object o) {
	...
	Ident i = null;
	VarExpr var = (VarExpr) ast.E1;
	SimpleVar v = (SimpleVar) var.V;
	i = v.I;
	Decl varDecl = (Decl) i.decl;

	if (varDecl.isGlobalVarDecl()) {
		emitPUTSTATIC(VCtoJavaType(varDecl.T), i.spelling);
  ...
  public Object visitSimpleVar(SimpleVar ast, Object o) {
	...
	if (d.isGlobalVarDecl()) {
		emitGETSTATIC(VCtoJavaType(d.T), ast.I.spelling);
	} else if (ast.type.isIntType() || ast.type.isBooleanType()) {
		emitILOAD(d.index);
	} else if (ast.type.isFloatType()) {
		emitFLOAD(d.index);
